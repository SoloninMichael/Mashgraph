...
GLUSvoid GLUSAPIENTRY glusCreateSpheref(GLUSshape* shape, GLUSfloat radius, GLUSuint numberSlices)
{
	...
	for (i = 0; i < numberParallels + 1; i++ )
	{
		for (j = 0; j < numberSlices + 1; j++ )
		{
			...
			//int tt = rand() % 10;
			shape->vertices[vertexIndex + 3] = min(shape->vertices[vertexIndex + 2], min(shape->vertices[vertexIndex + 0], shape->vertices[vertexIndex + 1]));
...
GLUSvoid GLUSAPIENTRY glusCreateSpheref2(GLUSshape* shape, GLUSfloat radius, GLUSuint numberSlices)
{
	GLUSuint i, j;
	radius /= 1.4;
	...
	GLUSuint numberParallels = numberSlices;
	GLUSuint numberVertices = (numberParallels + 1) * (numberSlices + 1);
	GLUSuint numberIndices = 2 * numberParallels * numberSlices * 6;
