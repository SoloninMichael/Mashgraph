...
GLUSshaderprogram my_g_program;
SimpleMesh* my_g_simpleMesh = NULL;
...
GLUSboolean init(GLUSvoid)
{
  try 
  {
	...
	std::ifstream my_geomSourceFile("../main/myGeometry.geom");
	...
	my_geomSourceFile.seekg(0, std::ios::end);
	my_geomSource.reserve(my_geomSourceFile.tellg());
	my_geomSourceFile.seekg(0, std::ios::beg);
	my_geomSource.assign((std::istreambuf_iterator<char>(my_geomSourceFile)), std::istreambuf_iterator<char>());
	...
	const char* my_tmpGeomSource = my_geomSource.c_str();
	...
	if (!glusBuildProgram(&my_g_program,
		(const GLUSchar**)&tmpVertSource,  // vertex shader source
		NULL,                              // tessellation shader source
		NULL,                              // evaluation shader source
		(const GLUSchar**)&my_tmpGeomSource,  // geometry shader source
		(const GLUSchar**)&tmpFragSource)) // fragment shader source
		throw std::runtime_error("shader compilation failed!");
    ...
    g_simpleMesh = new SimpleMesh(g_program.program, true);

    // GL init done. Now init some our internal stuff, related to materials, cameras and other
    //
	my_g_simpleMesh = new SimpleMesh(my_g_program.program, false);
    return GLUS_TRUE;
  }
...
GLUSboolean update(GLUSfloat time)
{
  try 
  {
	...
	setUniform(g_program.program, "tri", 1);
	setUniform(g_program.program, "g_camPos", g_camPos);
	setUniform(g_program.program, "LightType", LightType);


	//glUseProgram(my_g_program.program);  CHECK_GL_ERRORS;
	//setUniform(my_g_program.program, "modelViewMatrix", modelView);
	//setUniform(my_g_program.program, "projectionMatrix", g_projectionMatrix);            // set matrix we have calculated in "reshape" funtion

	//setUniform(my_g_program.program, "timeParameter", 0.0075f*elaspedTimeFromStart);
    ...
    g_simpleMesh->Draw(GL_TRIANGLES);
	...
	setUniform(my_g_program.program, "timeParameter", 0.0075f*elaspedTimeFromStart);
	setUniform(my_g_program.program, "tri", 0);
	setUniform(my_g_program.program, "g_camPos", g_camPos);
	setUniform(my_g_program.program, "LightType", LightType);
	my_g_simpleMesh->Draw(GL_LINES);
