...
SimpleMesh::SimpleMesh(GLuint a_programId, GLboolean tri)
{
  if (tri)
	 glusCreateSpheref(&m_glusShape, 1.0f, 150);
  else
	  glusCreateSpheref2(&m_glusShape, 1.0f, 150);
...
void SimpleMesh::Draw(GLuint type)
{
  glBindVertexArray(m_vertexArrayObject); CHECK_GL_ERRORS;
  glDrawElements(type, m_glusShape.numberIndices, GL_UNSIGNED_INT, 0);  CHECK_GL_ERRORS;
}
